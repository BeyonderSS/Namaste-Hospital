...
import { TbReportSearch, TbPrescription } from "react-icons/tb";
      ...
      >
        <div className="">
          ...
          <div
            ...
            className="flex items-center text-sm md:text-base py-2 px-4 lg:px-8 rounded-lg font-medium cursor-pointer hover:bg-gray-200 transition-colors duration-300"
          >
            <p>{session ? `Hello, ${session.user.name}` : "Sign In"}</p>
            {session && (
              <button
                className="bg-blue-500 text-white ml-4 py-1 px-3 lg:px-6 rounded-md text-sm tracking-wide hover:bg-blue-600 transition-colors duration-300"
                onClick={signOut}
              >
                Sign Out
              </button>
